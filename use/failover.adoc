...
[role="lead"]
En cas d'incident, basculez votre site primaire sur site vers VMware Cloud sur un site AWS avec FSX pour NetApp ONTAP.
...
Ce processus ne consomme pas de capacité physique supplémentaire pour Amazon FSX for NetApp ONTAP. Le volume source d'origine n'est pas modifié et les tâches de réplica peuvent se poursuivre même en cas de reprise après sinistre.
...
En cas d'incident, basculez votre site source à la demande vers VMware Cloud sur un site AWS avec FSX pour NetApp ONTAP.
...
* Les datastores sont créés et montés sur le cluster ou l'hôte VMC en fonction des informations capturées dans le plan de reprise d'activité.
...
TIP: Une fois le basculement lancé, les éléments récupérés sont visibles dans le vCenter VMC (machines virtuelles, réseaux et datastores). Par défaut, les machines virtuelles sont restaurées dans le dossier charge de travail.
...
. Sur la page Test du basculement, entrez le nom du plan de reprise d'activité à confirmer et sélectionnez *basculement*.
